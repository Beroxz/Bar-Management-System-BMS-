...
if (isset($_GET['booking']) && $_GET['booking']=="del"){	

  $table_id  = mysqli_real_escape_string($condb,$_GET["table_id"]);
	$sql_del = "UPDATE tbl_table SET table_status = '0' WHERE table_id=$table_id";
	$result_del = mysqli_query($condb, $sql_del) or die ("Error in query: $sql_del ". mysqli_error());

  $sql_del2 = "UPDATE tbl_booking SET table_id = '0' WHERE table_id=$table_id";
  ...
  echo "<script type ='text/javascript'>";
  echo "window.location = 'index.php?book_no=book_no';";
  echo "</script>";
