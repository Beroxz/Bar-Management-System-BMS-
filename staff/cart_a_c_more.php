...
<form id="frmcart" name="frmcart" method="post" action="?t_name=<?php echo $t_name;?>&act=update">
...
if(!empty($_SESSION['cart']))
{
	...
	foreach($_SESSION['cart'] as $p_id=>$qty)
	{
		...
		echo "<input type='number' name='amount[$p_id]' value='$qty' size='2'class='form-control' min='1'max='$p_qty'/>";


		echo "<td align='right'>".number_format($sum,2)."</td>";

		//remove product
		echo "<td align='center'><a href='cart.php?p_id=$p_id&act=remove' class='btn btn-danger btn-xs'>ลบ</a></td>";
        echo "</tr>";
...
<!-- <a href="#" target="" class="btn btn-success" onclick="window.print()">Print</a> -->
	...
	<input type="button" name="btcancel" id="buttonu" value="ยกเลิกออเดอร์" class="btn btn-danger" onclick="window.location='cart_more.php?t_name=<?php echo $t_name;?>&act=cancel';" />
	<input type="submit" name="buttonu" id="buttonu" value="อัพเดตรายการ" class="btn btn-warning" />
	<?php if($act=='update'){ ?>
		<input type="button" name="Submit2" value="สั่งซื้อ" onclick="window.location='cart_view_more.php?t_name=<?php echo $t_name;?>';" class="btn btn-primary" />	
	<?php } ?>
